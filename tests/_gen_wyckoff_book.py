...
def has_drawing(par):
    return par._p.find('.//' + '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}drawing') is not None
...
# ---- normalization map (applied to all text) ----
NORM = [
...
# event number repair (after the above, fix "事件 N2 2" etc.)
EVENT_RE = re.compile(r'事件\s*(?:编号|N\s*\d\s*)?\s*#?\s*(\d+)')
EVENT_RE2 = re.compile(r'事件\s*N\s*(\d)\s*(\d)')   # "事件 N2 2" -> #22 ? actually maps to #2
EVENT_RE3 = re.compile(r'件\s*#\s*(\d+)')
...
out.append('<p class="lede">以下内容逐章整理自 Rubén Villahermosa Chaves 的《The Wyckoff Methodology in Depth》（2019）中文译本，作为本体系「威科夫操盘法」的延伸阅读。原文为机器翻译，术语已尽量统一为站内约定（吸筹 / 派发、弹簧 Spring、强势信号 SOS、弱势信号 SOW、最后支撑点 LPS、震仓 Shakeout、上冲 Upthrust、自动反弹 AR 等），个别表述保留原译。原书配图此处从略，相关形态请参见本页上方的 TradingView 示意图。</p>')
...
for i in range(start_idx, len(paras)):
    ...
    txt = p.text.strip()
    if not txt:
        # a drawing-only paragraph?
        if has_drawing(p):
            flush_bullets()
            out.append('<figure class="book-fig"><p class="muted small">〔原书配图略〕</p></figure>')
        continue
    bold, sz = runs_info(p)
    ...
    # stop at back matter / appendix (only after we've seen the real content)
    if cur_part >= 1 and re.search(r'该作者的其他著作|参考书目|参考文献|书目|索引|关于作者|版权所有|我们在网络上再见', t):
        break
    # bibliography lines: a 《title》 together with a （YYYY） year citation
    ...
    if is_part:
        ...
        chapters.append({'id': 'bk-p%d' % cur_part, 'label': label})
        continue
    if is_chap:
        flush_bullets()
        ...
        continue
    # section heading: bold + size>=16 + reasonably short
    ...
    if t.startswith('●') or t.startswith('•') or t.startswith('· '):
        ...
        continue
    # normal paragraph (text is kept even if the paragraph also carries an inline drawing)
    flush_bullets()
...
print("PARTS/CHAPTERS found:", len(chapters))
print("HTML blocks:", len(out))
for c in chapters:
    print("  ", c['id'], c['label'])
